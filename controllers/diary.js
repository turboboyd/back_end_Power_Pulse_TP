...
const listDiary = async (req, res) => {
  ...
  const products = await DiaryProduct.find(query, { amount: 1, calories: 1, product:1, _id: 1 });
  const exercises = await DiaryExercise.find(query, { time: 1, calories: 1, exercise:1, _id: 1 });
